...
foreach( $lists->movies as $list ){
	...
	if ( $list->movie->is_live == true && $list->movie->is_protected == false ){
		...
		$ret[] = array("url" => $list->movie->hls_url, "id" => $list->movie->id, "title" => $list->movie->title, "subtitle" => $list->movie->subtitle, "image" => $list->broadcaster->image, "name" => $list->broadcaster->name, "screen_id" => $list->broadcaster->screen_id);
